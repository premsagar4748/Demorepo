...
export class Api {
  ...
  postmethodXpert(endpoint: string, obj: object): Observable<any> {
    ...
    getMethodST<T>(endpoint: string): Observable<T> {
    return this.http.get<T>(`${environment.dev2conn}${endpoint}`);
  }
  ...
  headermenu(roleId: any) {
    ...
    return this.http.post( 'https://westerninsightsapi.axelone.app/api/cmsmodules/GetCmsPermissionsMenu', data, httpOptions)
  }
