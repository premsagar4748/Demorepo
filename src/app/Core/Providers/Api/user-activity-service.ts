...
export class UserActivityService {
  userID: any = '';
  private baseUrl = 'https://westernccapi.axelone.app/api/';
  ...
  private getLoginUserId(): string | null {
    ...
    try {
      ...
      const token = storage?.Login_token || btoa(JSON.stringify(storage?.userInfo));
  ...
  private refreshToken(): Observable<string> {
    ...
    if (!host.includes('localhost') && !host.includes('127.0.0.1')) {
      ...
      const prodToken = CCcaptureObject?.Login_token;
