...
export class Dashboard implements OnInit {
    ...
    this.shared.api.postmethod('Western/GetStoresList', obj).subscribe({
      next: (res: any) => {
        ...
        if (res) {
          ...
          this.storeIds = this.stores.map((s: { ID: any; }) => s.ID);
  ...
  updateStoreButtonLabel(): void {
    if (this.storeIds.length === this.stores.length) {
      ...
      const selected = this.stores.find((s: { ID: any; }) => s.ID === this.storeIds[0]);
  ...
  allstores(state: 'Y' | 'N') {
    this.storeIds = state === 'Y' ? this.stores.map((s: { ID: any; }) => s.ID) : [];
    this.updateStoreSelection();
  }
  ...
  ngAfterViewInit(): void {
    this.shared.api.getStores().subscribe((res: any) => {
      if (this.shared.common.pageName == 'Account Mapping') {
...
if (!storeValue && selectedStoreIds.length) {
  storeValue = selectedStoreIds.join(', ');
    ...
    const reportMonth = this.frommonth
      ...
      : '-';
      worksheet.addRow(['Report Stores:', storeValue]);
