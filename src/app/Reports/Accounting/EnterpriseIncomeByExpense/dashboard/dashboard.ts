...
@Component({
  ...
  standalone: true,
  imports: [SharedModule, BsDatepickerModule, Stores,ToastContainer],
...
export class Dashboard {
  ...
  selectedLabel: string = 'Department (All)';
  ...
  updateLabel() {
    ...
    if (this.selectedFilters.length === this.filters.length) {
      this.selectedLabel = 'Department (All)';
    }
    else if (this.selectedFilters.length === 1) {
      this.selectedLabel = `Department (${this.selectedFilters[0]})`;
    }
    else {
      this.selectedLabel = `Department (Selected ${this.selectedFilters.length})`;
    }
  ...
  ngAfterViewInit(): void {
    this.shared.api.getStores().subscribe((res: any) => {
      if (this.shared.common.pageName == 'Enterprise Income / Expense') {
