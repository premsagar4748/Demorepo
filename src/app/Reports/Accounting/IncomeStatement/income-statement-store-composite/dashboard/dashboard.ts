...
export class Dashboard {
            ...
            if (x.response != undefined) {
              if (x.response.length > 0) {
                ...
                let ETByStoreKeys_sorted = this.XpenseTrendByStoreKeys.filter((x) => {
                  ...
                  // When hiding budget, exclude all budget/variance keys including total
  ...
  openDetails(object: any, storename: string, ref: any, item: any) {

    let selectedStoreId: any = '';
  
    // If clicked from store row → find store ID
    if (storename && storename !== 'ALL STORES') {
      const storeObj = this.stores.find(
        (s: any) => s.storename === storename
      );
      selectedStoreId = storeObj ? storeObj.ID : '';
    }
    ...
    console.log('Selected Store ID:', this.Fsdetails.STORES);
  ...
  ngAfterViewInit(): void {
    this.shared.api.getStores().subscribe((res: any) => {
      if (this.shared.common.pageName == 'Income Statement Store Composite') {
  ...
  exportToExcel(): void {
    ...
    const formattedMonth = this.shared.datePipe.transform(this.Month, 'MMMM-yyyy'); // ===== STORE VALUE FOR EXCEL (FROM UI SELECTION) =====
