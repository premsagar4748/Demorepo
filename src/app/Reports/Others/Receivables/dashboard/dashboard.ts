...
export class Dashboard {
  ...
  constructor(
    ...
    if (this.shared.common.groupsandstores.length > 0) {
      this.groupsArray = this.shared.common.groupsandstores.filter(
        (val: any) => val.sg_id != this.shared.common.reconID,
      );
      this.stores = this.shared.common.groupsandstores.filter(
        (v: any) => v.sg_id == this.groupId,
      )[0].Stores;
      this.StoreVal.length == this.stores.length
        ? (this.groupName = this.stores[0].sg_Name)
        : (this.groupName = '');
      this.StoreVal.length == 1
        ? (this.storename = this.stores.filter((e: any) => e.ID == this.StoreVal)[0].storename)
        : (this.storename = '');
  ...
  exportToExcel(): void {
    ...
    summarySheet.addRow(['Receivables']);
    ...
    workbook.xlsx.writeBuffer().then(() => {
      this.shared.exportToExcel(workbook, 'Vehicle Flooring_Report');
    });
  }
